...
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/ai/generate-resume-from-text`, {
        method: 'POST',
        headers: await getAuthHeaders(),
        body: JSON.stringify({
          profile_id: activeProfileId,
...
    } catch (err) {
      set({ makerAiError: err.message + " (Note: This requires a new backend endpoint: /api/v1/ai/generate-resume-from-text)" });
    } finally {
...
    console.log("handleRemoveResume is deprecated.");
    get().addNotification("File storage is no longer used. Simply clear the resume text and re-save.", "info");
    return;
  },
  
